...
const notificationSetRouter = require('../route/notification-set');


class Server {

  constructor(config, logger, database, sender) {
    this.config   = config;
  ...
  _setupExpressMiddleware() {
    ...
    this.expressApp.request.sendNotification = (...args) => this.sender.send(...args);
  ...
  _setupExpressRoutes() {
    ...
    this.expressApp.use('/', notificationSetRouter);
